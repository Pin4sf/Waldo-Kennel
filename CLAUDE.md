...
**start with the inherited "clone agent-orchestrator verbatim" banner at the top**, which
governs the current look.

The current renderer was derived from the **agent-orchestrator web app**
...
diverges from its approved donor baseline — do **not** re-flag old design-reference mismatches. This is a foundation-era implementation constraint, not a decision that AO's product model or visual language defines future Waldo features.
