...
- v0 dogfood admits Codex first through the provider-neutral adapter contract (amended 2026-08-24: DeepSeek Harness is additionally admitted for ordinary worker sessions under capability-based admission; this locked Local Focus Ledger fixture itself remains Codex-first). This does not choose the v1 provider set.
...
| **Act & Observe** | Kennel admits the authorized provider (Codex-first; DeepSeek eligible for workers), creates the isolated worktree, records Attempt/session identity, and presents next action plus Needs You/Action Required/Waiting only when warranted. | Attempt, AgentSessionRef, fence/lease facts, ordered observations, recovery receipt. | Healthy run needs no transcript; injected loss reconciles before a replacement Attempt; stale events cannot mutate current truth. |
