...
- Composed Outcomes: decomposition into contributing Outcomes, criterion-bound contribution, proof roll-up, and batched acceptance are accepted in [ADR 0007](adr/0007-composed-outcomes.md) and unimplemented.
- The Work navigation hierarchy. `/work` is a four-stage linear wizard over one Outcome, not Project Board -> Outcome Mission Control -> Session Inspector.
- An adaptive Mission. `ProposePlan` derives one direct WorkUnit deterministically from the contract; nothing is model-proposed, branches, or carries dependencies.
