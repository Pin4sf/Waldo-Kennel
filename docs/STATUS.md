...
The launch-provider decision is now Codex-only for new v1 work so the team can test the end-to-end responsibility loop with less provider variability. Other inherited provider identities remain readable for historical records but are non-selectable for new Attempts. The exact Codex admission matrix and historical non-Codex inspect/resume/handoff contract remain unapproved.
...
These documents do not make the prototype Outcome overlay, PR #11's Outcome schema, or any Mission/verification/acceptance feature shipped. The implementation sequence begins with accepting F0-F6 and replacing PR #11 with bounded post-foundation cleanup according to the [PR convergence and architecture gate plan](superpowers/plans/2026-08-20-pr-convergence-and-architecture-gate.md). Permanent Outcome persistence waits for the remaining Codex admission/historical recovery, RunBrief, observability, evaluation, and consequential-effect decisions.
