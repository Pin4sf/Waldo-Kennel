...
A proposed [Learning and Skill Evolution design](superpowers/specs/2026-08-21-waldo-learning-skill-evolution-design.md) and [ADR 0005](adr/0005-governed-project-learning-and-skill-evolution.md) now specify the remaining Paxel/AutoResearch boundary: consented Project-attributed LearningEpisodes propose candidates; bounded campaigns evaluate one editable surface against baseline, held-in, hidden held-out, and adversarial work; only the user may promote an exact SkillRevision and scope. Learning foundations may run in shadow alongside Work/Home, but no skill activates until trustworthy Outcome result facts, evaluation, tool/privacy contracts, and rollback pass. The written specification is awaiting review and is not shipped or accepted product authority yet.
...
These documents do not make the prototype Outcome overlay, PR #11's rejected Outcome schema, or any Home/Mission/Open Loop/capture/memory/communication/verification/acceptance/learning feature shipped. The prerequisite sequence is complete: F0-F6 landed in PR #1, legacy import removal in PR #12, provider-neutral admission in PR #13, and CLI reduction in PR #14. PR #11 is closed unmerged as a superseded donor. The [Local Focus Ledger Outcome](product/kennel-v0-first-outcome-slice.md) remains the first complete Work milestone. ADR 0004 permits a separate Home/Personal Agent lane to execute in parallel through an explicit file/API ownership matrix. Proposed ADR 0005 would add a shadow Learning lane, while active skills remain behind trustworthy result, held-out evaluation, promotion, and rollback gates. Durable admitted Memory remains separately gated. Each code slice still requires its own approved implementation plan and issue/PR authority.
