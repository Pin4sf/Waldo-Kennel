...
| A2-2 | Non-repository (supplied document) context with an explicit custody model | `service/intelligence/supplied_context.go` | **implemented adapter; Outcome integration open** `782e75c3d` | explicit bounded text formats, secret/symlink/special-file refusal, no Git claim; wire owner selection into intake/custody | `supplied_context_test.go` | automated adapter |
| A2-3 | Bounded context enters both Contract and Plan input digests | Contract and Plan intelligence requests serialize `RepositoryContextSnapshot` | **implemented at request digest; document Outcome integration open** `782e75c3d` | bind selected snapshot through the actual document intake path | `intake_adapter_test.go` plus supplied snapshot digest test | automated source |
...
| B1 | Board and List as equivalent Outcome projections with filters and columns | `OutcomesOverviewSurface.tsx` | **partial** `2b5fc669e` | shared Outcome overview now has title search, Project filter, and Board/List projection; attention/history columns and daemon-backed filters remain | renderer typecheck; existing overview tests | automated/source |
...
| C foundation + classification | `5670e1cc7`, `67f6daa74`, `646de9f73` | prior `go test ./...` plus focused R checks; migration **0119** ledgered; sqlc regenerated | C-1 … C-11 closed; R corrections follow in `646de9f73` |
...
| Retention review correction | `0d41cb6c4`, `d96fc3aff` | artifactstore focused test, `git diff --check` pass | bounded file reads now fail closed if content grows beyond the total retention limit; existing published versions are verified before reuse |
| G integration | `59aa8703a`, `d02d77ae5`, `0d41cb6c4`, `d96fc3aff`, `1f77c04f2`, `f40d3f6b4` | bootstrap, sqlc, api parity, build, vet, full Go tests, full Go race, lint, frontend typecheck/tests/build, package identity | local branch is clean and reviewable; [Luna reviewer handoff](2026-09-10-luna-review-guide.md); live provider, daemon-backed journey, full canonical C-13/D/A2/E integration, and owner acceptance remain open |
...
| Final code head before evidence-only updates | `1f77c04f2` |
