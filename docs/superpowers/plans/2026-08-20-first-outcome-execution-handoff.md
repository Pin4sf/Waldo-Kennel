...
**Goal:** Resolve PR #1/#11 safely, then deliver one complete Local Focus Ledger Outcome through Enter → Understand → Decide & Authorize → Act & Observe → Prove & Close.
...
- **Proposed implementation detail:** filenames, Go interfaces, route/component names, migration numbers, and exact PR cuts below. The implementing agent must reconcile them with the accepted foundation base and may narrow them without changing the locked contract.
- **Unknown until execution evidence:** the final accepted foundation SHA, whether PR #1's full gate is green, exact cleanup donor set from PR #11, implementation effort, and whether the first-slice trials justify continuing to Home.

## Global constraints

- This plan does not authorize implementation, branch mutation outside a new worktree, push, merge, deploy, publish, or release.
- Start every implementation branch from the accepted PR #1 head or its eventual merge commit, never from this docs branch or PR #11.
- PR #11 is a donor diff only. Never cherry-pick or merge it wholesale.
...
## Current observed PR facts

Read-only refresh on 2026-08-20:

| Item | Observed state |
| --- | --- |
| PR #1 | Open, mergeable, `mergeStateStatus=BLOCKED`, head `ef0baffd7850702480cba84ae97e2790367eab12`, no checks reported. |
| PR #11 | Open sibling from pre-foundation `main`, head `dbebb0956d2e039fefc77bb963c6ef9bc0c7e28b`, no checks reported. |
| `origin/main` | `b1bcd37fd0911a86da5bb21c07ac3a0b9adecb3b`. |
| PR relationship | PR #1 and PR #11 share the pre-foundation main commit; PR #11 does not contain F0-F6. |

Refresh these facts at execution time. If any SHA or state differs, stop before creating branches and amend this handoff with the new evidence.

---

### Task 1: Resolve the foundation gate and PR #11 disposition
...
### Task 2: Land bounded cleanup prerequisites
...
2. Refresh PR #1/#11 facts and foundation test evidence.
3. Obtain explicit user authority for exactly one issue/PR task.
4. Create a fresh `codex/` worktree from the accepted foundation base.
