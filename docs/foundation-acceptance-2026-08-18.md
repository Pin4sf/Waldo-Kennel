...
This gate covers repository provenance, Kennel identity and state isolation, an inherited migration collision, local verification/security controls, deterministic bootstrap and dependency policy, and truthful documentation.
...
| F4 verification and security | Local controls complete; hosted CI deferred | The repository keeps deterministic local commands for bootstrap, production audits, foundation checks, lint, race tests, and macOS package identity. Dependabot and GitHub secret-scanning controls remain configured. The hosted CI/security workflows and their required status checks were intentionally deferred by the maintainer on 2026-08-21 and will be reintroduced separately. |
...
- Desktop frontend: 191 test files, 2,488 passed tests and 6 intentional skips, plus typecheck/build coverage in the gate.
...
The full development dependency audit is not clean because inherited build/test toolchains remain: root 3 advisories (1 moderate, 2 high), cloud 2 high, desktop 36 (3 low, 1 moderate, 31 high, 1 critical), and scripts 4 (1 moderate, 3 high). Product UI, ACP runtime, and landing have zero. These advisories are explicit dev-toolchain debt; the local foundation gate fails on shipped production dependency vulnerabilities, while hosted security automation remains deferred.
