...
func (m *Manager) validateRuntimePrerequisites() error {
	...
	if path, err := m.lookPath("tmux"); err != nil || path == "" {
		return fmt.Errorf("%w: %s", ports.ErrRuntimePrerequisite, tmuxInstallGuidance(runtime.GOOS))
	}
	return nil
}

// tmuxInstallGuidance is intentionally returned with the prerequisite error so
// every client (desktop, CLI, and API consumer) receives an actionable install
// option without the daemon ever installing software on the user's behalf.
func tmuxInstallGuidance(goos string) string {
	switch goos {
	case "darwin":
		return "tmux is required to start Terminal UI sessions but was not found in PATH. Install it with Homebrew: brew install tmux, then retry"
	case "linux":
		return "tmux is required to start Terminal UI sessions but was not found in PATH. Install it with your distribution package manager (for example: sudo apt install tmux), then retry"
	default:
		return "tmux is required to start Terminal UI sessions but was not found in PATH. Install tmux, then retry"
	}
}
