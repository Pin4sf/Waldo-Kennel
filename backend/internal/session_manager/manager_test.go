...
func TestSpawn_ResolvesProjectConfig(t *testing.T) {
	...
	if rt.lastCfg.Env[EnvSessionID] == "" {
		t.Fatal("runtime env missing KENNEL_SESSION_ID")
	}
...
func TestSpawn_WrapsSupervisedAgentAndPersistsGeneration(t *testing.T) {
	...
	wantArgv := []string{"/opt/kennel", "agent-process", "supervise", "--session", "mer-1", "--launch", "launch-7", "--", "codex", "--model", "gpt-5"}
...
func TestRestore_RotatesSupervisedAgentGeneration(t *testing.T) {
	...
	wantArgv := []string{"/opt/kennel", "agent-process", "supervise", "--session", "mer-1", "--launch", "launch-new", "--", "codex", "resume", "agent-x"}
	if !reflect.DeepEqual(rt.lastCfg.Argv, wantArgv) {
...
// in `kennel session get` (created in local time, updated in UTC).
...
func TestSpawn_WorkspaceProjectRecordsRootAndChildWorktrees(t *testing.T) {
	...
	if rec.Metadata.Branch != "kennel/mer-1" {
...
func TestKill_WorkspaceProjectDestroysChildrenBeforeRoot(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		ID:        "mer-1",
		ProjectID: "mer",
		Metadata:  domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1", RuntimeHandleID: "h1"},
		Activity:  domain.Activity{State: domain.ActivityActive},
	}
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1"},
		{SessionID: "mer-1", RepoName: "api", Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1/api"},
	}
...
func TestKill_WorkspaceProjectFailsClosedOnUnregisteredChildRows(t *testing.T) {
	...
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1"},
		{SessionID: "mer-1", RepoName: "old-api", Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1/old-api"},
		{SessionID: "mer-1", RepoName: "api", Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1/api"},
	}
...
func TestRestore_WorkspaceProjectRestoresChildrenAndRecordsInventory(t *testing.T) {
	...
	seedTerminal(st, "mer-1", domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1", AgentSessionID: "agent-x"})
...
func TestCleanup_WorkspaceProjectDestroysChildrenBeforeRoot(t *testing.T) {
	...
	st.projects["mer"] = domain.ProjectRecord{ID: "mer", Path: "/repo/mer", Kind: domain.ProjectKindWorkspace, Config: testRoleAgents()}
	st.workspaceRepo["mer"] = []domain.WorkspaceRepoRecord{{Name: "api", RelativePath: "api"}}
	seedTerminal(st, "mer-1", domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1"})
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1"},
		{SessionID: "mer-1", RepoName: "api", Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1/api"},
	}
...
func TestSpawn_DefaultsBranchFromSessionID(t *testing.T) {
	...
	if got := st.sessions[s.ID].Metadata.Branch; got != "kennel/mer-1/root" {
...
func TestSpawn_DefaultsBranchUnderDevNamespaceForDevDataDir(t *testing.T) {
	...
	t.Setenv("HOME", home)
	m, st, _, _ := newManager()
	m.dataDir = filepath.Join(home, ".kennel", "dev", "data")

	worker, _, _, err := m.Spawn(ctx, ports.SpawnConfig{ProjectID: "mer", Kind: domain.KindWorker})
	if err != nil {
		t.Fatal(err)
	}
	if got := st.sessions[worker.ID].Metadata.Branch; got != "kennel/dev/mer-1/root" {
		t.Fatalf("worker branch = %q, want kennel/dev/mer-1/root", got)
	}
	...
	if got := st.sessions[orchestrator.ID].Metadata.Branch; got != "kennel/dev/mer-orchestrator" {
		t.Fatalf("orchestrator branch = %q, want kennel/dev/mer-orchestrator", got)
	}
}
...
func TestRestore_ForwardsResolvedAgentConfigPermissions(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		...
		Kind:         domain.KindWorker,
		IsTerminated: true,
		Metadata:     domain.SessionMetadata{Branch: "kennel/mer-1", WorkspacePath: "/tmp/ws", AgentSessionID: "native-1"},
	}
	agent := &recordingAgent{}
...
func TestSpawnWorker_AppendsActiveOrchestratorContact(t *testing.T) {
	...
	for _, want := range []string{
		...
		`kennel send --session mer-1 --message "<your message>"`,
...
func TestSpawnWorker_SkipsTerminatedOrchestratorContact(t *testing.T) {
	...
	systemPrompt := agent.lastLaunch.SystemPrompt
	if strings.Contains(systemPrompt, "## Orchestrator Coordination") || strings.Contains(systemPrompt, "kennel send --session mer-1") {
...
func TestSpawnOrchestrator_UsesCoordinatorPrompt(t *testing.T) {
	...
	for _, want := range []string{
		...
		`kennel spawn --project mer --name "<label>" --prompt "<clear worker task>"`,
		"Before running `kennel spawn`, count the `--name` label yourself",
		...
		"Use `kennel send` for session communication",
		"`kennel session ls --project mer`",
...
func TestRestore_PromptlessOrchestratorResumesViaAdapter(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		...
		Metadata: domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-orchestrator"},
		Activity: domain.Activity{State: domain.ActivityExited},
	}
	rt := &fakeRuntime{}
	lookPath := func(string) (string, error) { return "/bin/true", nil }
...
func TestRestore_PromptlessWorkerNotResumable(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		...
		Metadata: domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1/root"},
		Activity: domain.Activity{State: domain.ActivityExited},
	}
	rt := &fakeRuntime{}
	lookPath := func(string) (string, error) { return "/bin/true", nil }
...
func TestRestore_WorkerPointsAtCurrentOrchestrator(t *testing.T) {
	...
	if !strings.Contains(agent.lastRestore.SystemPrompt, `kennel send --session mer-9`) {
...
func TestSpawn_RejectsMissingTmuxBeforeSessionRow(t *testing.T) {
	...
	_, _, _, err := m.Spawn(ctx, ports.SpawnConfig{ProjectID: "mer", Kind: domain.KindWorker})
	if !errors.Is(err, ports.ErrRuntimePrerequisite) || !strings.Contains(err.Error(), "tmux is required") {
		t.Fatalf("err = %v, want missing tmux prerequisite", err)
	}
	if runtime.GOOS == "darwin" && !strings.Contains(err.Error(), "brew install tmux") {
		t.Fatalf("err = %v, want Homebrew install guidance", err)
	}
	if len(st.sessions) != 0 {
...
func TestSpawnAndRestore_PinHookPATHToDaemonBinary(t *testing.T) {
	daemonExe := filepath.Join(t.TempDir(), "kennel")
...
// silent: when the daemon executable cannot anchor `kennel` resolution, PATH is
...
func TestSpawn_DoesNotAddNodeRuntimeForNativeBinary(t *testing.T) {
	...
	m := New(Deps{
		...
		Executable: func() (string, error) { return "/kennel/bin/kennel", nil },
	})
	if _, _, _, err := m.Spawn(ctx, ports.SpawnConfig{ProjectID: "mer", Kind: domain.KindWorker}); err != nil {
		t.Fatalf("Spawn: %v", err)
	}
	...
	want := strings.Join([]string{binDir, "/kennel/bin", "/usr/bin"}, string(os.PathListSeparator))
	if got := rt.lastCfg.Env["PATH"]; got != want {
		t.Fatalf("runtime env PATH = %q, want %q", got, want)
	}
}
...
func TestSaveAndTeardownAll_CaptureOrderAndMarker(t *testing.T) {
	...
	if rows[0].PreservedRef != "refs/kennel/preserved/mer-1" {
		t.Fatalf("preserved_ref = %q, want refs/kennel/preserved/mer-1", rows[0].PreservedRef)
	}
...
func TestRetireForReplacementWorkspaceProjectClosesScopedShellTerminalsBeforeForceDestroy(t *testing.T) {
	...
	st.worktrees["mer-orch"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-orch", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-orchestrator", WorktreePath: "/ws/mer-orch", State: "active"},
		{SessionID: "mer-orch", RepoName: "api", Branch: "kennel/mer-orchestrator", WorktreePath: "/ws/mer-orch/api", State: "active"},
	}
	...
	st.worktrees["mer-orch"] = []domain.SessionWorktreeRecord{
		{
			...
			Branch:       "kennel/mer-orchestrator",
			WorktreePath: "/ws/mer-orch",
			PreservedRef: "refs/kennel/preserved/old-root",
			State:        "active",
		},
		{
			SessionID:    "mer-orch",
			...
			PreservedRef: "refs/kennel/preserved/old-api",
			State:        "active",
		},
	}
...
func TestSaveAndTeardownAll_NoKindFilter(t *testing.T) {
	...
	st.sessions["mer-2"] = domain.SessionRecord{
		...
		Metadata: domain.SessionMetadata{WorkspacePath: "/ws/mer-2", Branch: "kennel/mer-orchestrator", RuntimeHandleID: "h2"},
		Activity: domain.Activity{State: domain.ActivityActive},
	}
...
func TestSaveAndTeardownAll_WorkspaceProjectPreservesEachRepoAndRemovesChildrenFirst(t *testing.T) {
	...
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1", BaseSHA: "root-base"},
		{SessionID: "mer-1", RepoName: "api", Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1/api", BaseSHA: "api-base"},
	}
	...
	if refs[domain.RootWorkspaceRepoName] != "refs/kennel/preserved/mer-1/__root__" || refs["api"] != "refs/kennel/preserved/mer-1/api" {
...
func TestSaveAndTeardownAll_WorkspaceProjectRegistryDriftPreservesWholeWorkspace(t *testing.T) {
	...
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-1/root", WorktreePath: "/ws/mer-1", State: "active"},
		{SessionID: "mer-1", RepoName: "old-child", Branch: "kennel/mer-1/root", WorktreePath: "/ws/mer-1/old-child", State: "active"},
	}
...
func TestRestoreAll_RestoresBothWorkerAndOrchestrator(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		...
		Harness:      domain.HarnessClaudeCode,
		IsTerminated: true,
		Metadata:     domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1/root", AgentSessionID: "agent-w"},
		Activity:     domain.Activity{State: domain.ActivityExited},
	}
	st.sessions["mer-2"] = domain.SessionRecord{
		ID:           "mer-2",
		...
		Harness:      domain.HarnessClaudeCode,
		IsTerminated: true,
		Metadata:     domain.SessionMetadata{WorkspacePath: "/ws/mer-2", Branch: "kennel/mer-orchestrator", AgentSessionID: "agent-o"},
		Activity:     domain.Activity{State: domain.ActivityExited},
	}
...
func TestRestoreAll_SkipsSessionsKilledBeforeShutdown(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		...
		Harness:      domain.HarnessClaudeCode,
		IsTerminated: true,
		Metadata:     domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1/root", Prompt: "do it"},
		Activity:     domain.Activity{State: domain.ActivityExited},
	}
...
func TestRestoreAll_SkipsActiveWorkspaceProjectRowsFromUserKilledSession(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		...
		Harness:      domain.HarnessClaudeCode,
		IsTerminated: true,
		Metadata:     domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1", Prompt: "do it"},
		Activity:     domain.Activity{State: domain.ActivityExited},
	}
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1", State: "active"},
		{SessionID: "mer-1", RepoName: "api", Branch: "kennel/mer-1", WorktreePath: "/ws/mer-1/api", State: "active"},
	}
...
func TestRestoreAll_AppliesPreservedRef(t *testing.T) {
	...
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: "__root__", PreservedRef: "refs/kennel/preserved/mer-1", State: "removed"},
	}
...
func TestRestoreAll_WorkspaceProjectRestoresAndAppliesEachRepo(t *testing.T) {
	...
	st.sessions["mer-1"] = domain.SessionRecord{
		...
		Harness:      domain.HarnessClaudeCode,
		IsTerminated: true,
		Metadata:     domain.SessionMetadata{WorkspacePath: "/ws/mer-1", Branch: "kennel/mer-1", AgentSessionID: "agent-w"},
		Activity:     domain.Activity{State: domain.ActivityExited},
	}
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{
		{SessionID: "mer-1", RepoName: domain.RootWorkspaceRepoName, Branch: "kennel/mer-1", BaseRef: "refs/remotes/origin/main", WorktreePath: "/ws/mer-1", PreservedRef: "refs/kennel/preserved/mer-1", State: "removed"},
		{SessionID: "mer-1", RepoName: "api", Branch: "kennel/mer-1", BaseRef: "refs/remotes/origin/dev", WorktreePath: "/ws/mer-1/api", PreservedRef: "refs/kennel/preserved/mer-1", State: "removed"},
	}
	...
	if !strings.Contains(applied, "ApplyPreserved:__root__:refs/kennel/preserved/mer-1") ||
		!strings.Contains(applied, "ApplyPreserved:api:refs/kennel/preserved/mer-1") {
...
func TestRestoreAll_WorkspaceProjectRootOnlyMarkerRestoresRegisteredChildren(t *testing.T) {
	...
	st.worktrees["mer-1"] = []domain.SessionWorktreeRecord{{
		...
		PreservedRef: "refs/kennel/preserved/root",
		State:        "removed",
	}}
	...
	applied := strings.Join(ws.calls, ",")
	if !strings.Contains(applied, "ApplyPreserved:__root__:refs/kennel/preserved/root") {
...
func TestReconcileLive_DeadSessionStashedAndTerminated(t *testing.T) {
	...
	ws := &fakeWorkspace{stashRef: "refs/kennel/preserved/s1"}
	lcm := &fakeLCM{store: st}
	lookPath := func(string) (string, error) { return "/bin/true", nil }
	...
	rec := domain.SessionRecord{
		...
		IsTerminated: false,
		Metadata: domain.SessionMetadata{
			Branch: "kennel/s1/root", WorkspacePath: "/wt/s1", RuntimeHandleID: "s1",
		},
	}
	...
	if len(rows) != 1 || rows[0].PreservedRef != "refs/kennel/preserved/s1" {
...
func TestReconcileLive_AliveSessionAdoptedNoop(t *testing.T) {
	...
	rec := domain.SessionRecord{
		...
		Metadata: domain.SessionMetadata{Branch: "kennel/s2/root", WorkspacePath: "/wt/s2", RuntimeHandleID: "s2"},
	}
...
func TestReconcileLive_ProbeErrorIsNotDeath(t *testing.T) {
	...
	rec := domain.SessionRecord{
		...
		IsTerminated: false,
		Metadata: domain.SessionMetadata{
			Branch: "kennel/s3/root", WorkspacePath: "/wt/s3", RuntimeHandleID: "s3",
		},
	}
...
func TestReconcileLive_ScratchDeadRuntimeTerminatesWithoutWorkspaceTeardown(t *testing.T) {
	...
	ws := &fakeWorkspace{stashRef: "refs/kennel/preserved/scratch-1"}
	lcm := &fakeLCM{store: st}
	m := New(Deps{
		Runtime:   rt,
		Agents:    fakeAgents{},
...
func TestReconcile_AdoptAcrossDaemonRestart(t *testing.T) {
	...
	ws := &fakeWorkspace{stashRef: "refs/kennel/preserved/mer-3"}
	lcm := &fakeLCM{store: st}
	lookPath := func(string) (string, error) { return "/bin/true", nil }
	m := New(Deps{Runtime: rt, Agents: fakeAgents{}, Workspace: ws, Store: st, Messenger: &fakeMessenger{}, Lifecycle: lcm, LookPath: lookPath})
