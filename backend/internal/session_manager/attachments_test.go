...
func TestWriteSpawnAttachments(t *testing.T) {
	...
	refs, err := m.writeSpawnAttachments(context.Background(), "kennel-1", dir, []ports.SpawnAttachment{
	...
	want := []string{".kennel/attachments/attachment-1.html", ".kennel/attachments/attachment-2.png", ".kennel/attachments/attachment-3.bin"}
...
func TestStageAttachmentsUsesNeutralFileNames(t *testing.T) {
	...
	refs, err := m.StageAttachments(context.Background(), "kennel-1", []ports.SpawnAttachment{
	...
	if !strings.HasPrefix(refs[0], ".kennel/attachments/attachment-") || !strings.HasSuffix(refs[0], ".html") {
	...
	if _, err := os.Stat(filepath.Join(dataDir, "attachments", "kennel-1", filepath.Base(refs[0]))); err != nil {
...
func TestStageAttachmentsRetriesGeneratedNameCollisionsWithoutOverwritingHistory(t *testing.T) {
	...
	for _, dir := range []string{
		filepath.Join(dataDir, "attachments", "kennel-1"),
	...
	refs, err := m.StageAttachments(context.Background(), "kennel-1", []ports.SpawnAttachment{{Ext: ".png", Data: []byte("new bytes")}})
	if err != nil {
		t.Fatalf("StageAttachments: %v", err)
	}
	if len(refs) != 1 || refs[0] != ".kennel/attachments/attachment-cafebabe00.png" {
...
func TestAppendAttachmentReferences(t *testing.T) {
	t.Run("appends after a brief", func(t *testing.T) {
		got := appendAttachmentReferences("Fix the button", []string{".kennel/attachments/attachment-1.html"})
		if !strings.HasPrefix(got, "Fix the button\n\n") {
			t.Errorf("brief not preserved: %q", got)
		}
		if !strings.Contains(got, "- .kennel/attachments/attachment-1.html") {
			t.Errorf("missing reference: %q", got)
		}
	})

	t.Run("handles empty brief", func(t *testing.T) {
		got := appendAttachmentReferences("", []string{".kennel/attachments/attachment-1.html"})
