...
func TestSessionList_ProjectFilterAndDefaultFiltering(t *testing.T) {
	...
	if !strings.Contains(out, "2 orchestrator sessions hidden. Use --all or `kennel orchestrator ls` to show.") {
...
func TestSessionList_HintsWhenOnlyTerminatedOrchestratorIsHidden(t *testing.T) {
	...
	if !strings.Contains(out, "1 orchestrator session hidden. Use --all or `kennel orchestrator ls` to show.") {
...
func TestSessionClaimPR_ProjectScopeMismatchIsUsage(t *testing.T) {
	...
	_, _, err := executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }}, "session", "claim-pr", "demo-1", "https://github.com/Pin4sf/Waldo-Kennel/pull/142", "-p", "other")
...
func TestSessionClaimPR_UsesCurrentSessionFromEnv(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("claim-pr with KENNEL_SESSION_ID failed: %v stderr=%s", err, errOut)
	}
...
func TestSessionClaimPR_OneArgRequiresCurrentSessionEnv(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "pass <session-id> or set KENNEL_SESSION_ID") {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestSessionClaimPR_HelpDocumentsCurrentSessionShorthand(t *testing.T) {
	...
	for _, want := range []string{
		...
		"current session is read from KENNEL_SESSION_ID",
...
func TestSessionClaimPR_JSONAndNoTakeoverError(t *testing.T) {
	...
	out, errOut, err := executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }}, "session", "claim-pr", "demo-1", "https://github.com/Pin4sf/Waldo-Kennel/pull/142", "--json")
	if err != nil {
	...
	_, _, err = executeCLI(t, Deps{ProcessAlive: func(int) bool { return true }}, "session", "claim-pr", "demo-1", "https://github.com/Pin4sf/Waldo-Kennel/pull/142", "--no-takeover")
...
func TestSessionClaimPR_GHFallbackWhenProjectRepoMissing(t *testing.T) {
	...
	out, _, err := executeCLI(t, Deps{
		...
		CommandOutputInDir: func(_ context.Context, dir, name string, args ...string) ([]byte, error) {
			...
			return []byte("https://github.com/Pin4sf/Waldo-Kennel\n"), nil
		},
