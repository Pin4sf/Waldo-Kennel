...
const appBundleName = "Kennel.app"

// appStateFileName is the marker the desktop app writes under ~/.kennel on every
// launch (spec §5). `kennel start` is a read-only consumer of it.
const appStateFileName = "app-state.json"

// appState mirrors the app-written ~/.kennel/app-state.json marker (spec §5). Only
// the desktop app writes it; `kennel start` reads it as a fast-path hint and never
...
// startResult is the JSON shape emitted with --json: what `kennel start` resolved,
...
func newStartCommand(ctx *commandContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "start",
		Short: "Fetch (if needed) and open the Kennel desktop app",
		Long: "Fetch (if needed) and open the Kennel desktop app.\n\n" +
			"The desktop app now owns the daemon, state, and updates. `kennel start` no\n" +
...
// at itself, and `kennel start` would then keep reopening that copy instead of the
...
// markerAppPath reads ~/.kennel/app-state.json and returns its recorded appPath, or
...
// aoStateDir resolves the canonical ~/.kennel home, honoring KENNEL_DATA_DIR exactly as
// the daemon's config does (the marker lives beside running.json under ~/.kennel).
func aoStateDir() (string, error) {
	cfg, err := config.Load()
	if err != nil {
		return "", err
	}
	// running.json lives directly under ~/.kennel; the marker sits beside it.
...
func knownAppLocations() []string {
	...
	case "windows":
		...
		if pf := os.Getenv("ProgramFiles"); pf != "" {
			paths = append(paths, filepath.Join(pf, "Kennel", "kennel.exe"))
		}
		return paths
	case "linux":
		paths := []string{linuxAppImagePath()}
		if home, err := os.UserHomeDir(); err == nil {
			paths = append(paths, filepath.Join(home, "Applications", "kennel.AppImage"))
		}
		return paths
	default:
		return nil
...
func windowsInstalledExe(localAppData string) string {
	return filepath.Join(localAppData, "Programs", "Kennel", "kennel.exe")
}

// linuxAppImagePath is the stable location `kennel start` downloads the AppImage to
...
func linuxAppImagePath() string {
	...
	if err != nil {
		...
		return "kennel.AppImage"
	}
	return filepath.Join(dir, "kennel.AppImage")
}
...
Terminal=false
NoDisplay=true
MimeType=x-scheme-handler/kennel-app;
`, desktopExecPath(appPath))
	...
	if out, err := c.deps.CommandOutput(
		...
		return fmt.Errorf(
			"register kennel-app URL handler with xdg-mime (install xdg-utils or use the .deb/.rpm package): %w: %s",
			err,
			out,
		)
	}
	...
	if err != nil {
		return fmt.Errorf("verify kennel-app URL handler: %w: %s", err, out)
	}
	if strings.TrimSpace(string(out)) != linuxDesktopEntryName {
		return fmt.Errorf(
			"verify kennel-app URL handler: xdg-mime selected %q instead of %q",
...
func (c *commandContext) fetchApp(ctx context.Context, w io.Writer) (string, error) {
	...
	default:
		return "", fmt.Errorf("kennel start: fetch not supported on %s", runtime.GOOS)
	}
}

// fetchAppDarwin downloads the latest macOS release zip and unpacks it into a
// staging dir under ~/.kennel/staging, returning the .app bundle path (spec §6.3).
func (c *commandContext) fetchAppDarwin(ctx context.Context, w io.Writer) (string, error) {
	...
	// it so a quiet `kennel start` doesn't look hung after the download finishes.
	...
	if !isUsableBundle(appPath) {
		return "", fmt.Errorf("kennel start: %s not found in unpacked release at %s", appBundleName, staging)
	}
	return appPath, nil
}
...
func (c *commandContext) fetchAppWindows(ctx context.Context, w io.Writer) (string, error) {
	...
	if local == "" {
		return "", fmt.Errorf("kennel start: LOCALAPPDATA not set; cannot locate installed app")
	}
	appPath := windowsInstalledExe(local)
	if !isUsableBundle(appPath) {
		return "", fmt.Errorf("kennel start: installed app not found at %s", appPath)
	}
	return appPath, nil
}

// fetchAppLinux downloads the self-contained AppImage to a stable path under
// ~/.kennel, makes it executable, and returns it. runStart separately installs the
...
func (c *commandContext) fetchAppLinux(ctx context.Context, w io.Writer) (string, error) {
	...
	if !isUsableBundle(appPath) {
		return "", fmt.Errorf("kennel start: AppImage not found at %s", appPath)
	}
	return appPath, nil
}
...
func (c *commandContext) download(ctx context.Context, w io.Writer, url, asset, dst string) error {
	...
	if total > 0 {
		_, _ = fmt.Fprintf(w, "Downloading Kennel (%s, ~%s) from %s...\n", asset, humanBytes(total), releaseRepo)
	} else {
		_, _ = fmt.Fprintf(w, "Downloading Kennel (%s) from %s...\n", asset, releaseRepo)
	}
...
//   - darwin: kennel-darwin-{arm64,x64}.zip (signed bundle zip)
//   - windows: kennel-win32-x64.exe (NSIS installer, amd64 only)
//   - linux: kennel-linux-x64.AppImage (self-contained, amd64 only)
func assetName() (string, error) {
	...
	case "darwin":
		...
		return fmt.Sprintf("kennel-darwin-%s.zip", arch), nil
	case "windows":
		if _, err := requireAMD64(); err != nil {
			return "", err
		}
		return "kennel-win32-x64.exe", nil
	case "linux":
		if _, err := requireAMD64(); err != nil {
			return "", err
		}
		return "kennel-linux-x64.AppImage", nil
	default:
		return "", fmt.Errorf("kennel start: no release asset for %s", runtime.GOOS)
	}
}
...
func requireAMD64() (string, error) {
	if runtime.GOARCH != "amd64" {
		return "", fmt.Errorf("kennel start: unsupported architecture %q on %s (only amd64 is published)", runtime.GOARCH, runtime.GOOS)
	}
	return "x64", nil
}
...
func assetArch(goarch string) (string, error) {
	...
	case "amd64":
		return "x64", nil
	default:
		return "", fmt.Errorf("kennel start: unsupported architecture %q", goarch)
	}
}
...
func (c *commandContext) openApp(ctx context.Context, appPath string) (bool, error) {
	...
	case "windows", "linux":
		...
		// detached, so `kennel start` does not block on the app. StartProcess uses
...
func (c *commandContext) printDeprecationNotice(w io.Writer) {
	_, _ = fmt.Fprint(w, "Kennel is distributed as a desktop app.\n"+
		"The app is distributed from the website and GitHub Releases; it owns the daemon and updates itself.\n"+
		"You can keep running `kennel start` to fetch (if needed) and open it.\n")
}

// printManualOpen tells the user how to open the bundle when `kennel start` could
