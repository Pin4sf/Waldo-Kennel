...
func TestDelegateOutcomeSendsApprovalGatedContractToOrchestrator(t *testing.T) {
	st := newFakeStore()
	st.projects["ao"] = domain.ProjectRecord{ID: "ao"}
	...
	out, err := (&Service{store: st, manager: cmd}).DelegateTask(context.Background(), DelegateTaskInput{
		ProjectID: "ao", Brief: "Ship reliable offline mode", Outcome: true, RequestedAgent: domain.HarnessCodex,
	})
	if err != nil {
		t.Fatalf("DelegateTask outcome: %v", err)
	}
	if out.OrchestratorID != "orch" || out.WorkerID != "orch" || cmd.spawned {
		t.Fatalf("out=%#v spawned=%v, want existing orchestrator and no worker", out, cmd.spawned)
	}
	if len(cmd.ready) != 1 || len(cmd.sentMessages) != 1 {
		t.Fatalf("ready=%#v messages=%#v", cmd.ready, cmd.sentMessages)
	}
	if got := st.sessions["orch"].DisplayName; got != "Outcome: Ship reliable offline mode" {
		t.Fatalf("orchestrator display name = %q", got)
	}
	for _, want := range []string{
		...
		"must not delegate",
		"Preferred worker harness after approval: codex",
	} {
		if !strings.Contains(cmd.sentMessages[0], want) {
			t.Fatalf("outcome intake missing %q:\n%s", want, cmd.sentMessages[0])
		}
	}
}

func TestOutcomeSessionDisplayNameIsCompactAndUnicodeSafe(t *testing.T) {
	got := outcomeSessionDisplayName(strings.Repeat("界", 100))
	if !strings.HasPrefix(got, "Outcome: ") || utf8.RuneCountInString(got) != outcomeSessionTitleLimit {
		t.Fatalf("outcome title = %q (%d runes)", got, utf8.RuneCountInString(got))
	}
}
