...
// chatRec marks a record as a Chat-mode session, whose activity reaches Kennel
...
func TestServiceDerivesStatusFromSessionFactsAndPR(t *testing.T) {
	tests := []struct {
		...
		// A chat session's silence is not evidence of anything. Kennel owns the
...
func TestOutcomeStatusMarkerMovesCoordinationAcrossKanban(t *testing.T) {
	tests := []struct {
		name   string
		marker string
		want   domain.SessionStatus
	}{
		{"working", "working", domain.StatusWorking},
		{"needs-you", "needs_you", domain.StatusNeedsInput},
		{"reviewing", "reviewing", domain.StatusReviewPending},
		{"ready-to-merge", "ready_to_merge", domain.StatusMergeable},
		{"finished-alias", "finished", domain.StatusMergeable},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rec := chatRec(statusRec(domain.ActivityIdle, false))
			rec.Kind = domain.KindOrchestrator
			rec.DisplayName = "Outcome: Ship lifecycle reporting"
			rec.Metadata.LatestAssistantUpdate = "Progress summary.\n" + outcomeStatusMarker + " " + tt.marker
			if got := deriveStatus(rec, nil, statusNow, true); got != tt.want {
...
func TestOutcomeStatusMarkerTrustBoundary(t *testing.T) {
	outcome := chatRec(statusRec(domain.ActivityIdle, false))
	outcome.Kind = domain.KindOrchestrator
	outcome.DisplayName = "Outcome: Ship lifecycle reporting"
	outcome.Metadata.LatestAssistantUpdate = "old: " + outcomeStatusMarker + " needs_you\nnow: " + outcomeStatusMarker + " ready_to_merge"

	if got := deriveStatus(outcome, nil, statusNow, true); got != domain.StatusMergeable {
		t.Fatalf("last marker = %q, want mergeable", got)
	}

	worker := chatRec(statusRec(domain.ActivityIdle, false))
	worker.Kind = domain.KindWorker
	worker.Metadata.LatestAssistantUpdate = workStatusMarker + " reviewing"
	if got := deriveStatus(worker, nil, statusNow, true); got != domain.StatusReviewPending {
		t.Fatalf("worker marker status = %q, want review_pending", got)
	}
	worker.Metadata.LatestAssistantUpdate = outcomeStatusMarker + " ready_to_merge"
	if got := deriveStatus(worker, nil, statusNow, true); got != domain.StatusIdle {
		t.Fatalf("worker accepted outcome-only marker as %q", got)
	}
	worker.Activity.State = domain.ActivityActive
	worker.Metadata.LatestAssistantUpdate = workStatusMarker + " ready_to_merge"
	if got := deriveStatus(worker, nil, statusNow, true); got != domain.StatusWorking {
		t.Fatalf("stale ready marker overrode active work with %q", got)
	}

	withPR := outcome
	if got := deriveStatus(withPR, statusPR(domain.PRFacts{CI: domain.CIFailing}), statusNow, true); got != domain.StatusCIFailed {
		t.Fatalf("marker overrode SCM status with %q", got)
	}

	terminated := outcome
	terminated.IsTerminated = true
	if got := deriveStatus(terminated, nil, statusNow, true); got != domain.StatusTerminated {
		t.Fatalf("marker overrode termination with %q", got)
	}
}
