	...
	return ports.PlanIntelligenceResponse{
		Proposal: domain.PlanDraftProposal{
			...
			WorkUnits: []domain.PlanDraftWorkUnit{
				{Key: "verify", Title: "Verify outcome", OutputSummary: "Verified result", CriteriaCovered: []string{"C2"}, DependsOn: []string{"edit"}, EvidenceIdeas: []string{"verification output"}},
				{Key: "edit", Title: "Implement outcome", OutputSummary: "Implemented result", CriteriaCovered: []string{"C1"}, EvidenceIdeas: []string{"workspace diff"}},
			},
		},
...
func newPlanningTestService(t *testing.T, router *routingInventoryFake) (*outcome.Service, *planningFakeStore, domain.OutcomeID, *twoUnitPlanIntelligence) {
	...
	view, err := svc.Create(ctx, outcome.CreateInput{
		...
		SuccessCriteria: []string{"Implementation is present."}, Review: "Run deterministic verification.", RequestKey: "req-plan-compiler",
	})
	if err != nil { t.Fatalf("seed outcome: %v", err) }
	view, err = svc.ReviseContract(ctx, view.Outcome.ID, outcome.ReviseContractInput{
		...
		Review: "Run deterministic verification.",
	})
	if err != nil { t.Fatalf("seed second criterion: %v", err) }
...
func TestProposePlanCompilesIntelligenceGraphAndRoutesEveryWorkUnit(t *testing.T) {
	...
	if err != nil { t.Fatalf("propose plan: %v", err) }
	if provider.calls != 1 { t.Fatalf("plan intelligence calls = %d, want 1", provider.calls) }
	if len(view.Plan.WorkUnits) != 2 || len(view.Plan.RoutingDecisions) != 2 { t.Fatalf("compiled plan = %+v", view.Plan) }
	ordered, err := view.Plan.TopologicalWorkUnits()
	if err != nil { t.Fatalf("topological order: %v", err) }
	if ordered[0].Title != "Implement outcome" || ordered[1].Title != "Verify outcome" { t.Fatalf("topological titles = %q -> %q", ordered[0].Title, ordered[1].Title) }
	for _, unit := range view.Plan.WorkUnits {
		if unit.Provider != domain.HarnessClaudeCode || unit.ModelSelection != domain.ExecutionBindingModelExplicit || unit.Model != "sonnet-test" { t.Fatalf("work unit %s binding = %s/%s/%s", unit.ID, unit.Provider, unit.ModelSelection, unit.Model) }
		if len(unit.CriterionIDs) != 1 || unit.CriterionIDs[0].IsZero() { t.Fatalf("work unit %s criteria = %+v", unit.ID, unit.CriterionIDs) }
	}
	if err := domain.ValidateExactPlanCapabilityGrants(view.Plan.Grants, view.Plan.WorkUnits); err != nil { t.Fatalf("compiled grants: %v", err) }
	if router.preference == nil || router.preference.Provider != string(domain.HarnessClaudeCode) || router.preference.Model != "sonnet-test" { t.Fatalf("preference = %+v", router.preference) }
	if router.calls != 2 { t.Fatalf("routing calls = %d, want 2", router.calls) }
	var runs []domain.IntelligenceRun
	for _, run := range store.runs { if run.Kind == domain.IntelligenceRunPlanDraft { runs = append(runs, run) } }
	if len(runs) != 1 || runs[0].Status != domain.IntelligenceRunFulfilled || runs[0].OutcomeID != outcomeID || runs[0].ContractRevisionID.IsZero() { t.Fatalf("plan runs = %+v", runs) }
}
...
func TestApprovePlanDoesNotRereadMutableProjectPreference(t *testing.T) {
	...
	if store.projectReads != reads { t.Fatalf("approval reread Project: %d -> %d", reads, store.projectReads) }
	for _, unit := range approved.Plan.WorkUnits { if unit.Provider != domain.HarnessClaudeCode || unit.Model != "sonnet-test" { t.Fatalf("approved binding changed: %+v", unit) } }
}
...
func TestProposePlanNoValidRoutePersistsNothing(t *testing.T) {
	...
	if err == nil { t.Fatal("no admissible route must fail closed") }
	if code := apiCode(t, err); code != "PLAN_NO_VALID_ROUTE" { t.Fatalf("code = %s", code) }
	if persisted := len(store.plans[outcomeID]); persisted != 0 { t.Fatalf("persisted %d plans", persisted) }
}
