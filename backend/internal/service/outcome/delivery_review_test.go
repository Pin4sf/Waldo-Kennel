...
// TestReconcileDeliveries_CannotTellACommittedTransferFromAnAbandonedOne is the
// reconciliation half of the same gap.
//
// A delivery whose row is still pending after a restart is failed as
// interrupted. That is the right default — an unproven transfer must not be
// reported as done — but the destination manifest records the exact Attempt,
// artifact version and AcceptanceDecision, and reconciliation does not read it.
// So a transfer that demonstrably completed is recorded as failed, with no
// durable evidence linking the two.
func TestReconcileDeliveries_CannotTellACommittedTransferFromAnAbandonedOne(t *testing.T) {
	h := newDeliveryReviewHarness(t)
	destination := filepath.Join(t.TempDir(), "restart-bundle")
	delivery, err := h.svc.RequestDelivery(context.Background(), h.outcomeID,
		h.request(destination, "dlv-restart"))
	if err != nil {
		t.Fatalf("request delivery: %v", err)
	}
	if delivery.State != domain.DeliverySucceeded {
		t.Fatalf("state = %q, want succeeded", delivery.State)
	}

	// Rewind the ledger to the state a crash between the rename and the
	// completion write would leave: bytes delivered, row still pending.
	pending := delivery
	pending.State, pending.CompletedAt, pending.ManifestPath = domain.DeliveryPending, nil, ""
	pending.FileCount, pending.ByteCount = 0, 0
	h.deliveries.byID[pending.ID], h.deliveries.byKey[pending.RequestKey] = pending, pending

	manifest := filepath.Join(destination, artifactstore.ManifestName)
	if _, statErr := os.Stat(manifest); statErr != nil {
		t.Fatalf("committed manifest missing before reconciliation: %v", statErr)
	}
	closed, err := h.svc.ReconcileDeliveries(context.Background())
	if err != nil {
		t.Fatalf("reconcile: %v", err)
	}
	if closed != 1 {
		t.Fatalf("reconciliation closed %d rows, want the pending one", closed)
	}

	// This is the finding. The ledger now says the transfer failed as
	// interrupted, while the destination holds the exact manifest naming this
	// Attempt, artifact version and AcceptanceDecision. Refusing to claim an
	// unproven success is right; not reading the evidence that would settle it
	// leaves the two records permanently disagreeing.
	after, err := h.svc.GetDelivery(context.Background(), h.outcomeID, delivery.ID)
	if err != nil {
		t.Fatalf("read delivery: %v", err)
	}
	if after.State != domain.DeliveryFailed || after.FailureCode != "DELIVERY_INTERRUPTED" {
		t.Fatalf("delivery = %+v, want failed/DELIVERY_INTERRUPTED", after)
	}
	if _, statErr := os.Stat(manifest); statErr != nil {
		t.Fatalf("reconciliation removed the committed manifest: %v", statErr)
	}
	// And the owner cannot resolve the disagreement by retrying: the committed
	// destination refuses a fresh request with an unrelated-looking conflict.
	retry, err := h.svc.RequestDelivery(context.Background(), h.outcomeID,
		h.request(destination, "dlv-restart-retry"))
	if err != nil {
		t.Fatalf("retry: %v", err)
	}
	if retry.State != domain.DeliveryFailed || retry.FailureCode != "DELIVERY_DESTINATION_CONFLICT" {
		t.Fatalf("retry = %+v, want failed/DELIVERY_DESTINATION_CONFLICT", retry)
	}
}
