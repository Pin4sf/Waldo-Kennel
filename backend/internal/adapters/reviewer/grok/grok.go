// Package grok adapts Grok Build as an experimental user-approved Kennel reviewer.
package grok
...
import (
	...
	workergrok "github.com/Pin4sf/Waldo-Kennel/backend/internal/adapters/agent/grok"
	"github.com/Pin4sf/Waldo-Kennel/backend/internal/domain"
	"github.com/Pin4sf/Waldo-Kennel/backend/internal/ports"
)

// HostTrustWarning documents that Grok relies on its normal interactive
// approval flow rather than an Kennel-enforced read-only sandbox.
const HostTrustWarning = "experimental user-approved reviewer: Grok uses its normal permission prompts because Kennel cannot enforce a read-only sandbox"
...
// ReviewCommand starts Grok with Kennel's reviewer role as native rules. It emits
...
func (r *Reviewer) ReviewCommand(ctx context.Context, inv ports.ReviewInvocation) (ports.ReviewCommandSpec, error) {
	...
	rules, err := os.ReadFile(inv.SystemPromptFile) //nolint:gosec // Kennel-owned prompt path
	if err != nil {
