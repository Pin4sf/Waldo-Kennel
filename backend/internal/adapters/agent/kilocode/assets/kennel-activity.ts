// kennel: managed kilocode activity plugin (do not edit)
//
...
export const aoActivity: Plugin = async ({ directory, client }) => {
  // kennel hooks must never be able to hang Kilo: cap each invocation, matching
  ...
  function hookCmd(hookName: string): string[] {
    return ["sh", "-c", `if ! command -v kennel >/dev/null 2>&1; then exit 0; fi; exec kennel hooks kilocode ${hookName}`]
  }
  ...
  function logHookFailure(hookName: string, detail: string) {
    try {
      void client?.app
        ?.log?.({ body: { service: "kennel-activity", level: "error", message: `hook ${hookName} failed: ${detail}` } })
        ?.catch?.(() => {})
    } catch {
  ...
  // `kennel hooks` failure) or a spawn exception is logged with its stderr and never
