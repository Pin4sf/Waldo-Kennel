...
export const aoActivity: Plugin = async ({ directory, client }) => {
  // kennel hooks must never be able to hang opencode: cap each invocation, matching
  ...
  function hookCmd(hookName: string): string[] {
    return ["sh", "-c", `if ! command -v kennel >/dev/null 2>&1; then exit 0; fi; exec kennel hooks opencode ${hookName}`]
  }
