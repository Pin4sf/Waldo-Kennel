...
const (
	...
	crushSystemPromptName   = "kennel-system-prompt.md"
	crushSystemPromptPath   = crushConfigDirName + "/" + crushSystemPromptName
	crushSystemPromptMarker = "kennel: managed crush system prompt"

	// crushModelSeparator splits Kennel's "<provider>/<model-id>" convention.
	// Crush's config schema (models.large / models.small) requires both a
	// provider id and a model id per selection; Kennel's AgentConfig.Model is a
	// single string, so Crush is the one adapter that needs a delimiter to
	// recover both parts (see mergeCrushModel).
	crushModelSeparator = "/"

	// crushModelType is the model type Kennel writes overrides into. Crush's
	// "large" model backs its main coder agent; Kennel has no equivalent of
	// Crush's separate "small"/task-agent model, so only large is managed.
	crushModelType = "large"
)

// GetAgentHooks installs Kennel's standing instructions as a Crush context file.
// Crush has no launch-time system-prompt flag, but it reads context_paths from
// project config. Kennel therefore owns one prompt file under .crush/ and merges
...
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	...
	if _, err := os.Stat(promptPath); err == nil {
		...
		if !managed {
			return fmt.Errorf("crush.GetAgentHooks: refusing to overwrite non-Kennel file at %s", promptPath)
		}
...
// UninstallHooks removes Kennel's Crush context file and context_paths entry. User
// config is preserved; Kennel removes only its exact path and only deletes the
// prompt file when it carries the Kennel marker.
...
// AreHooksInstalled reports whether Kennel's Crush system-prompt context file is
// present. Crush still lacks activity hooks; this reports only the prompt
// injection hook surface Kennel manages for Crush.
...
// (providers, options, other model type). Kennel owns only the model/provider
