...
const (
	...
	// (.github/hooks/*.json). Kennel writes a single dedicated file there so it never
	...
	// copilotHookCommandPrefix identifies the hook commands Kennel owns, so install
...
// copilotHookFile is the on-disk shape of .github/hooks/kennel.json. Kennel owns this
...
// copilotHookSpec describes one hook Kennel installs, defined in code rather than
...
type copilotHookSpec struct {
	...
	Event string
	// Command is the Kennel sub-command suffix (session-start, ...). It is appended
	// to copilotHookCommandPrefix to form both the bash and powershell command,
	// and is the value DeriveActivityState switches on.
	Command string
}

// copilotManagedHooks is the source of truth for the hooks Kennel installs. The Kennel
...
// event — the closest signal that Kennel's permission-request sub-command can
...
// InstallAgentProfile installs only Kennel's per-session Copilot custom-agent
...
// GetAgentHooks installs Kennel's Copilot workspace integration:
...
// writing a repository-root AGENTS.md here so Kennel does not compete with
...
// UninstallHooks removes Kennel's Copilot hooks from the workspace-local
...
// AreHooksInstalled reports whether any Kennel Copilot hook is present in the
...
// file struct with a nil hooks map (and the Kennel schema version, used on write).
...
// isCopilotManagedHook reports whether an entry is one Kennel owns, recognized by the
...
// removeCopilotManagedHooks strips Kennel hook entries from a slice, preserving
