...
func TestGetLaunchCommandOrdersFlagsAndProtectsPrompt(t *testing.T) {
	...
	want := []string{
		"/bin/prime-agent",
		"--no-session",
		"--extension", filepath.Join(dataDir, "agent-runtime", "prime-agent", "kennel-activity.ts"),
...
func TestGetLaunchCommandOmitsBlankOptionalArguments(t *testing.T) {
	...
	want := []string{"prime-agent", "--no-session", "--extension", filepath.Join(dataDir, "agent-runtime", "prime-agent", "kennel-activity.ts")}
	if !reflect.DeepEqual(cmd, want) {
