...
func (w WorkUnit) Validate() error {
	...
	if err := validateNonBlankStrings("evidence check", w.EvidenceChecks); err != nil { return err }
	if strings.TrimSpace(w.VerificationRequirement) == "" { return fmt.Errorf("work unit verification requirement is required") }
	if err := validateNonBlankStrings("stop condition", w.StopConditions); err != nil { return err }
	if err := validateWorkUnitIDs("dependency", w.ID, w.DependsOn); err != nil { return err }
	if err := validateCriterionIDs(w.CriterionIDs); err != nil { return err }
	if err := validateUniqueCapabilityNames(w.RequiredCapabilities); err != nil { return err }
	return nil
}

func validateNonBlankStrings(kind string, values []string) error {
	for i, value := range values {
		if strings.TrimSpace(value) == "" { return fmt.Errorf("%s %d is blank", kind, i+1) }
	}
	return nil
}
...
func validateWorkUnitIDs(kind string, self WorkUnitID, ids []WorkUnitID) error {
	...
	for _, id := range ids {
		if id.IsZero() { return fmt.Errorf("work unit %s is blank", kind) }
		if id == self { return fmt.Errorf("work unit %s cannot reference itself", kind) }
		if _, duplicate := seen[id]; duplicate { return fmt.Errorf("work unit repeats %s %q", kind, id) }
		seen[id] = struct{}{}
	}
	return nil
}
...
func validateCriterionIDs(ids []CriterionID) error {
	...
	for _, id := range ids {
		if id.IsZero() { return fmt.Errorf("work unit criterion id is blank") }
		if _, duplicate := seen[id]; duplicate { return fmt.Errorf("work unit repeats criterion %q", id) }
		seen[id] = struct{}{}
	}
	return nil
}
...
func validateUniqueCapabilityNames(names []string) error {
	...
	for _, raw := range names {
		...
		if name == "" { return fmt.Errorf("work unit required capability is blank") }
		if _, duplicate := seen[name]; duplicate { return fmt.Errorf("work unit repeats required capability %q", name) }
		seen[name] = struct{}{}
	}
	return nil
}
...
func (w WorkUnit) ExecutionBinding() (ExecutionBinding, error) {
	...
	if selection == "" { selection = ExecutionBindingModelHistoricalUnbound }
...
func (p PlanRevision) Validate() error {
	...
	if strings.TrimSpace(p.Summary) == "" { return fmt.Errorf("plan revision summary is required") }
	if len(p.WorkUnits) == 0 { return fmt.Errorf("plan revision requires at least one work unit") }
	if len(p.WorkUnits) > MaxCanonicalPlanWorkUnits { return fmt.Errorf("plan revision has %d work units; maximum is %d", len(p.WorkUnits), MaxCanonicalPlanWorkUnits) }

	units := make(map[WorkUnitID]WorkUnit, len(p.WorkUnits))
	for i, unit := range p.WorkUnits {
		if err := unit.Validate(); err != nil { return fmt.Errorf("work unit %d: %w", i+1, err) }
		if unit.ContractRevisionNumber != p.ContractRevisionNumber { return fmt.Errorf("work unit %s binds contract revision %d, plan binds %d", unit.ID, unit.ContractRevisionNumber, p.ContractRevisionNumber) }
		if _, duplicate := units[unit.ID]; duplicate { return fmt.Errorf("duplicate work unit id %q", unit.ID) }
		units[unit.ID] = unit
	}
	for _, unit := range p.WorkUnits {
		for _, dependency := range unit.DependsOn {
			if _, exists := units[dependency]; !exists { return fmt.Errorf("work unit %s depends on unknown work unit %s", unit.ID, dependency) }
		}
	}
	if _, err := p.TopologicalWorkUnits(); err != nil { return err }
	...
	for i, grant := range p.Grants {
		...
		if seenGrants[grant.Name] { return fmt.Errorf("duplicate capability grant %q", grant.Name) }
		seenGrants[grant.Name] = true
	}
	if !isSHA256Hex(p.RunBriefCoreDigest) { return fmt.Errorf("plan revision requires a SHA-256 run brief core digest") }
	return nil
}
...
func (p PlanRevision) TopologicalWorkUnits() ([]WorkUnit, error) {
	...
	for _, unit := range p.WorkUnits {
		if unit.ID.IsZero() { return nil, fmt.Errorf("plan contains blank work unit id") }
		if _, duplicate := units[unit.ID]; duplicate { return nil, fmt.Errorf("duplicate work unit id %q", unit.ID) }
		units[unit.ID] = unit
		indegree[unit.ID] = 0
	}
	for _, unit := range p.WorkUnits {
		seen := map[WorkUnitID]struct{}{}
		for _, dependency := range unit.DependsOn {
			if dependency.IsZero() || dependency == unit.ID { return nil, fmt.Errorf("work unit %s has invalid dependency %s", unit.ID, dependency) }
			if _, exists := units[dependency]; !exists { return nil, fmt.Errorf("work unit %s depends on unknown work unit %s", unit.ID, dependency) }
			if _, duplicate := seen[dependency]; duplicate { return nil, fmt.Errorf("work unit %s repeats dependency %s", unit.ID, dependency) }
	...
	if len(order) != len(units) { return nil, fmt.Errorf("plan work unit dependencies contain a cycle") }
	return order, nil
}
...
func (p PlanRevision) ValidateAgainstContract(revision ContractRevision) error {
	...
	for _, criterion := range revision.Criteria { valid[criterion.ID] = struct{}{} }
	if len(valid) == 0 { return fmt.Errorf("contract revision has no canonical criteria") }
	covered := map[CriterionID]struct{}{}
	for _, unit := range p.WorkUnits {
		for _, criterionID := range unit.CriterionIDs {
			if _, exists := valid[criterionID]; !exists { return fmt.Errorf("work unit %s covers criterion %s outside the bound contract revision", unit.ID, criterionID) }
			covered[criterionID] = struct{}{}
		}
	}
	var missing []string
	for criterionID := range valid {
		if _, ok := covered[criterionID]; !ok { missing = append(missing, criterionID.String()) }
	}
	sort.Strings(missing)
	if len(missing) > 0 { return fmt.Errorf("plan does not cover required contract criteria: %s", strings.Join(missing, ", ")) }
	return nil
}
...
func (p PlanRevision) ValidateForApproval(revision ContractRevision) error {
	...
	if err := p.ValidateAgainstContract(revision); err != nil { return err }

	routing := make(map[WorkUnitID]WorkUnitRoutingDecision, len(p.RoutingDecisions))
	for _, record := range p.RoutingDecisions {
		if _, duplicate := routing[record.WorkUnitID]; duplicate { return fmt.Errorf("duplicate routing decision for work unit %s", record.WorkUnitID) }
		routing[record.WorkUnitID] = record
	}
	for _, unit := range p.WorkUnits {
		if _, err := unit.ExecutionBindingForNewWork(); err != nil { return fmt.Errorf("work unit %s is not executable: %w", unit.ID, err) }
		record, ok := routing[unit.ID]
		if !ok { return fmt.Errorf("work unit %s has no routing decision", unit.ID) }
		if err := record.ValidateAgainst(unit); err != nil { return err }
		if missing := MissingCapabilitiesForWorkUnit(p.Grants, unit); len(missing) > 0 { return fmt.Errorf("work unit %s is missing required capability grants: %s", unit.ID, strings.Join(missing, ", ")) }
	}
	if len(routing) != len(p.WorkUnits) { return fmt.Errorf("plan has routing decisions for unknown work units") }
	return nil
}

func (p PlanRevision) BindsCurrentContract(currentRevision int64) bool { return p.ContractRevisionNumber == currentRevision }
...
func MissingCapabilitiesForWorkUnit(grants []CapabilityGrant, unit WorkUnit) []string {
	...
	var missing []string
	for _, name := range unit.RequiredCapabilities { if !present[name] { missing = append(missing, name) } }
	sort.Strings(missing)
	return missing
}
...
func ComputePlanRunBriefCoreDigest(revision ContractRevision, units []WorkUnit, grants []CapabilityGrant) (string, error) {
	...
	if err := plan.Validate(); err != nil { return "", fmt.Errorf("run brief plan: %w", err) }
	...
	for _, unit := range ordered {
		...
		for _, id := range unit.DependsOn { dependencies = append(dependencies, id.String()) }
		sort.Strings(dependencies)
		criteria := make([]string, 0, len(unit.CriterionIDs))
		for _, id := range unit.CriterionIDs { criteria = append(criteria, id.String()) }
	...
	for _, grant := range grants { grantNames = append(grantNames, grant.Name+"@"+grant.Scope) }
...
func sortedTrimmed(in []string) []string {
	...
	for _, v := range in { if v = strings.TrimSpace(v); v != "" { out = append(out, v) } }
	sort.Strings(out)
	return out
}
