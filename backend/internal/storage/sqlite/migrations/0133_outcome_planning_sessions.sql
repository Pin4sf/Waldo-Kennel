-- Durable, Contract-bound direct-API planning conversation. Repository context
-- is a frozen read-only packet; no row in this schema grants execution.
...
ALTER TABLE plan_revisions ADD COLUMN planning_session_id TEXT REFERENCES planning_sessions (id);
ALTER TABLE plan_revisions ADD COLUMN source_intelligence_run_id TEXT REFERENCES intelligence_runs (id);
CREATE UNIQUE INDEX idx_plan_revisions_planning_session
...
WHEN OLD.id <> NEW.id
     ...
     OR OLD.routing_decisions_json <> NEW.routing_decisions_json
...
-- Reuse outcome_updated so this migration does not rebuild the global CDC
-- event vocabulary. Payload.type lets clients invalidate planning specifically.
-- +goose StatementBegin
...
-- +goose Down
DROP TRIGGER IF EXISTS planning_sessions_cdc_update;
DROP TRIGGER IF EXISTS planning_sessions_cdc_insert;
...
DROP TRIGGER IF EXISTS plan_revisions_immutable_update;
DROP INDEX IF EXISTS idx_plan_revisions_planning_session;
ALTER TABLE plan_revisions DROP COLUMN source_intelligence_run_id;
ALTER TABLE plan_revisions DROP COLUMN planning_session_id;
DROP TABLE IF EXISTS planning_turns;
DROP INDEX IF EXISTS idx_planning_sessions_outcome_created;
DROP INDEX IF EXISTS idx_planning_sessions_one_active_outcome;
