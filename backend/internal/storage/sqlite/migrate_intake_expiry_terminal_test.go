...
func TestIntakeExpiryMigrationRepairsSplitStateWithoutKillingNewerRetry(t *testing.T) {
	...
	// Reproduce a profile that has every beta migration except 0112. This is
	// the state an affected install is in immediately before upgrading.
	upTo(t, db, 111)
	if _, err := db.Exec(`
...
`,
	...
	if err := migrate(db); err != nil {
		t.Fatalf("migrate profile through 0112: %v", err)
	}
