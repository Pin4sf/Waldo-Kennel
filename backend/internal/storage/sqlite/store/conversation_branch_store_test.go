...
func seedBranchTurns(t *testing.T, s *sqlite.Store, session domain.SessionRecord, conversation domain.ConversationRecord) {
	...
	projected, found, err := s.GetSession(ctx, session.ID)
	if err != nil || !found {
		t.Fatalf("GetSession after assistant settle: found=%v err=%v", found, err)
	}
	if projected.Metadata.LatestAssistantUpdate != "first answer" {
		t.Fatalf("latest assistant update = %q, want settled Chat message", projected.Metadata.LatestAssistantUpdate)
	}
	created, err = s.AppendUserMessage(ctx, conversation.ID, session.ID, "generation-root", domain.ConversationMessage{
