...
func TestBrowserStatusAndSnapshot(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, http.MethodGet, "/api/v1/browser/status?sessionId=kennel-1", "")
	if status != http.StatusOK || !containsAll(body, `"connected":true`, `"transport":"electron-webcontents-debugger"`) {
		t.Fatalf("status = %d body=%s", status, body)
	}
	body, status, _ = doRequest(t, srv, http.MethodPost, "/api/v1/browser/commands", `{"sessionId":"kennel-1","action":"snapshot","args":{"interactive":true}}`)
	...
	for _, action := range actions {
		t.Run(action, func(t *testing.T) {
			body, status, _ := doRequest(
				...
				`{"sessionId":"kennel-1","action":"`+action+`","args":{"probe":true}}`,
			)
...
func TestBrowserCommandValidationAndErrors(t *testing.T) {
	...
	body, status, _ := doRequest(t, srv, http.MethodPost, "/api/v1/browser/commands", `{"sessionId":"kennel-1","action":"eval"}`)
	...
	body, status, _ = doRequest(t, srv, http.MethodPost, "/api/v1/browser/commands", `{"sessionId":"kennel-1","action":"snapshot"}`)
	...
	body, status, _ = doRequest(t, srv, http.MethodPost, "/api/v1/browser/commands", `{"sessionId":"kennel-1","action":"click"}`)
