...
const buildForge = vi.fn<(forge: { dir: string }, options: any) => Promise<string[]>>(async () => [
	"/out/make/Kennel-0.10.3-arm64.dmg",
]);
...
const makeOptions = {
	dir: "/tmp/app/Kennel-darwin-arm64",
...
const APP_PATH = "/tmp/app/Kennel-darwin-arm64/Kennel.app";
...
describe("MakerDMG", () => {
	...
	it("builds a dmg target for the requested arch and forwards config", async () => {
		const maker = new MakerDMG({ appId: "in.heywaldo.kennel" }, ["darwin"]);
		await maker.prepareConfig(makeOptions.targetArch);
		const artifacts = await maker.make(makeOptions);

		expect(artifacts).toEqual(["/out/make/Kennel-0.10.3-arm64.dmg"]);
	...
	// yields "Kennel.app/Kennel.app", an outer bundle with
	...
	it("never returns a dmg blockmap among the artifacts", async () => {
		buildForge.mockResolvedValueOnce([
			"/out/make/Kennel-0.10.3-arm64.dmg",
			"/out/make/Kennel-0.10.3-arm64.dmg.blockmap",
		]);
		...
		expect(await new MakerDMG().make(makeOptions)).toEqual(["/out/make/Kennel-0.10.3-arm64.dmg"]);
	});
});
...
describe("sealDmg", () => {
	...
	it("throws when notary credentials are present but a signing identity is not", async () => {
		await expect(sealDmg(dmg, { KENNEL_NOTARY_PROFILE: "ao" })).rejects.toThrow(/no signing identity/);
		expect(commands).toEqual([]);
	});
	...
	it("falls back to the keychain identity prefix when only CSC_LINK is set", async () => {
		await sealDmg(dmg, { CSC_LINK: "base64p12", KENNEL_NOTARY_PROFILE: "ao" });
	...
	it("fails the build when signing fails and credentials were present", async () => {
		...
		await expect(sealDmg(dmg, { APPLE_SIGNING_IDENTITY: "id", KENNEL_NOTARY_PROFILE: "ao" })).rejects.toThrow(
			/no identity found/,
		);
	});
});
