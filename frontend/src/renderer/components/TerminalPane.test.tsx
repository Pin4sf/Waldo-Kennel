...
describe("TerminalPane empty states", () => {
	...
	it("shows a startup message when a selected session has no terminal handle yet", () => {
		...
		try {
			...
			expect(
				screen.getByText(
					"Preparing the worker terminal. This can take a moment while Kennel creates the workspace and starts the agent.",
				),
			).toBeInTheDocument();
	...
	it("shows orchestrator-specific startup copy for a pending orchestrator terminal", () => {
		...
		try {
			...
			expect(
				screen.getByText(
					"Preparing the orchestrator terminal. This can take a moment while Kennel creates the workspace and starts the agent.",
				),
			).toBeInTheDocument();
