...
const FALLBACK_REVIEWERS: AgentInfo[] = CORE_PROVIDER_IDS.map((id) => ({ id, label: agentLabel(id) }));

// Security/permission warnings must come from daemon-owned provider metadata.
// Keep the exported helper while callers migrate, but do not maintain a second
// hard-coded provider trust policy in React.
...
export function ReviewerSelect({
	...
	const options = buildRankedAgentOptions({
		supported: supported ?? FALLBACK_REVIEWERS,
		installed,
		authorized,
		fallbackAgents: FALLBACK_REVIEWERS,
	});
