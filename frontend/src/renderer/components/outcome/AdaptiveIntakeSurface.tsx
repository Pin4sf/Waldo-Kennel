...
export function AdaptiveIntakeSurface({ projectId, intakeId }: { projectId: string; intakeId?: string }) {
	...
	async function capture(event: FormEvent) {
		...
		setPending(true); setError(null);
		try {
			const { data, error: apiError } = await apiClient.POST("/api/v1/projects/{id}/intakes", { params: { path: { id: projectId } }, body: { sourceSurface: "work", statement: statement.trim(), requestKey: requestKey("capture") } });
			if (apiError) throw apiError;
	...
	async function confirm() {
		...
		try {
			...
			const confirmed = await apiClient.POST("/api/v1/intakes/{intakeId}/confirmation", { params: { path: { intakeId } }, body: { expectedProposalRevision: current.session.currentProposalRevision, requestKey: requestKey("confirm") } });
