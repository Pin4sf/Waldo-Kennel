...
import { isTmuxPrerequisiteError, spawnOrchestrator } from "../lib/spawn-orchestrator";
...
export function ShellTopbar({
	...
	const openOrchestrator = async () => {
		...
		void captureRendererEvent("kennel.renderer.orchestrator_open_requested", { project_id: projectId });
		if (orchestrator) {
			void navigate({
	...
	const installTmuxAndRetry = async () => {
		...
		try {
			...
			if (result.status === "failed") {
				setBoardSpawnError(result.message ?? t("shell.couldNotSpawn"));
				setTmuxInstallMessage(null);
			}
			...
			<div className="workspace-topbar-actions flex shrink-0 items-center" data-testid="workspace-topbar-actions">
				{!boardActionsInPanel && isProjectBoardRoute ? (
					<>
						...
						{boardSpawnError && canInstallTmux && isMac ? (
				...
				{isSessionRoute ? (
					<>
						...
						{!isOrchestrator ? (
							<Tooltip>
								<TooltipTrigger asChild>
									<span hidden className="hidden" style={noDragStyle}>
										<TopbarButton
											aria-label={t("shell.openOrchestrator")}
											className="topbar-control--labeled"
											data-priority="secondary"
