...
/** True only for the missing tmux prerequisite, which the desktop app can repair on macOS. */
export function isTmuxPrerequisiteError(error: unknown): error is OrchestratorSpawnError {
	return (
		error instanceof OrchestratorSpawnError &&
		error.code === "RUNTIME_PREREQUISITE_MISSING" &&
		error.message.toLowerCase().includes("tmux")
	);
}
...
export async function spawnOrchestrator(
	...
	void captureRendererEvent("kennel.renderer.orchestrator_spawn_requested", { project_id: projectId, source });
	try {
		...
		void captureRendererEvent("kennel.renderer.orchestrator_spawn_succeeded", { project_id: projectId, source });
		return data.orchestrator.id;
	} catch (err) {
		void captureRendererEvent("kennel.renderer.orchestrator_spawn_failed", { project_id: projectId, source });
		throw err;
	}
}
