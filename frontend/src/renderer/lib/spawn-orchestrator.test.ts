...
import { isChatPreflightError, isTmuxPrerequisiteError, OrchestratorSpawnError, spawnOrchestrator } from "./spawn-orchestrator";
...
describe("spawnOrchestrator", () => {
	...
	it("identifies only the missing tmux runtime prerequisite as repairable", () => {
		expect(isTmuxPrerequisiteError(new OrchestratorSpawnError("tmux was not found", "RUNTIME_PREREQUISITE_MISSING"))).toBe(true);
		expect(isTmuxPrerequisiteError(new OrchestratorSpawnError("git was not found", "RUNTIME_PREREQUISITE_MISSING"))).toBe(false);
		expect(isTmuxPrerequisiteError(new OrchestratorSpawnError("tmux was not found", "CHAT_DRIVER_UNAVAILABLE"))).toBe(false);
	});
});
