...
const POSTHOG_KEY = import.meta.env.VITE_KENNEL_POSTHOG_KEY?.trim() || DEFAULT_POSTHOG_PROJECT_KEY;
const POSTHOG_HOST = import.meta.env.VITE_KENNEL_POSTHOG_HOST?.trim() || DEFAULT_POSTHOG_HOST;
...
const ACTIVE_STORAGE_KEY = "kennel.telemetry.activeSlotsByDate";
const ROUTE_VIEW_STORAGE_KEY = "kennel.telemetry.routeViewsByDate";
...
export async function sanitizeRendererProperties(
	...
	const safe: TelemetryProperties = {};
	switch (event) {
		case "kennel.app.active":
			if (properties?.channel === "renderer") safe.channel = "renderer";
			break;
		case "kennel.renderer.route_viewed":
			if (typeof properties?.surface === "string" && properties.surface.trim() !== "") {
				safe.surface = properties.surface;
			}
			break;
		case "kennel.renderer.project_add_requested":
		case "kennel.renderer.loaded":
			break;
		case "kennel.renderer.project_add_succeeded":
		case "kennel.renderer.project_removed":
		case "kennel.renderer.orchestrator_open_requested":
		case "kennel.renderer.task_create_requested":
		case "kennel.renderer.task_create_succeeded":
		case "kennel.renderer.task_create_failed":
		case "kennel.renderer.session_kill_requested":
		case "kennel.renderer.session_kill_succeeded":
		case "kennel.renderer.session_kill_failed":
		case "kennel.renderer.settings_save_requested":
		case "kennel.renderer.settings_save_succeeded":
		case "kennel.renderer.settings_save_failed": {
			const projectIDHash = await hashedTelemetryID(properties?.project_id);
			if (projectIDHash) safe.project_id_hash = projectIDHash;
			break;
		}
		case "kennel.renderer.orchestrator_spawn_requested":
		case "kennel.renderer.orchestrator_spawn_succeeded":
		case "kennel.renderer.orchestrator_spawn_failed": {
			const projectIDHash = await hashedTelemetryID(properties?.project_id);
			if (projectIDHash) safe.project_id_hash = projectIDHash;
		...
		case "kennel.renderer.notification_opened":
			if (properties?.target === "pr" || properties?.target === "session") safe.target = properties.target;
			break;
		case "kennel.renderer.notification_mark_read_requested":
		case "kennel.renderer.notification_mark_read_succeeded":
		case "kennel.renderer.notification_mark_read_failed":
			if (properties?.scope === "single" || properties?.scope === "all") safe.scope = properties.scope;
			break;
		case "kennel.renderer.daemon_failure":
			...
			break;
		case "kennel.renderer.api_error":
			...
			break;
		case "kennel.renderer.terminal_attach_failed":
		...
		case "kennel.renderer.session_state_unknown":
			...
			break;
		case "kennel.renderer.update_failed":
		case "kennel.renderer.update_downloaded":
		case "kennel.renderer.update_unsupported":
			...
			break;
		case "kennel.renderer.mobile_connect_opened":
			...
			break;
		case "kennel.renderer.update_channel_changed":
			...
			break;
		case "kennel.renderer.support_opened":
			break;
		case "kennel.renderer.support_submitted":
			...
			break;
		case "kennel.renderer.mobile_bridge_toggled":
...
export async function initTelemetry(): Promise<boolean> {
	...
	const attempt = (async () => {
		...
		startDailyActiveHeartbeat({
			...
			capture: async () =>
				isDeniedEvent("kennel.app.active")
					? true
					: Boolean(
					posthog.capture(
						postHogEventName("kennel.app.active"),
						withTelemetryContext(await sanitizeRendererProperties("kennel.app.active", { channel: "renderer" })),
					),
				),
		});
		if (!isDeniedEvent("kennel.renderer.loaded")) {
			posthog.capture(
				postHogEventName("kennel.renderer.loaded"),
				withTelemetryContext(await sanitizeRendererProperties("kennel.renderer.loaded")),
			);
		}
		return true;
	})().catch(() => {
...
export async function captureRendererEvent(event: string, properties?: Record<string, unknown>): Promise<void> {
	...
	if (event === "kennel.renderer.route_viewed") {
