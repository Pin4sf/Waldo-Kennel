 ...
 * the KENNEL_KEEP_DAEMON env var. When set to an explicit truthy value ("1",
 ...
 * an explicit `kennel stop`. Any other value — including conventional falsy ones
 ...
 */
export function keepDaemonAlive(env: { KENNEL_KEEP_DAEMON?: string }): boolean {
	const raw = env.KENNEL_KEEP_DAEMON?.trim().toLowerCase();
 ...
 * stay unlinked even when the app is later reopened without KENNEL_KEEP_DAEMON).
 * Only a normal app-owned daemon ("app") is linked; a keep-alive daemon
 * ("persistent") and headless `kennel start` daemons (owner unset/empty) stay
