...
export function getMacInstallBlocker(): string | undefined {
  ...
  // .../Kennel.app/Contents/MacOS/<binary> -> the .app bundle root
  ...
  if (bundle.includes("/AppTranslocation/")) {
    return (
      "macOS is running Kennel from a temporary read-only location " +
      "because it was opened straight from where it was downloaded. Quit the app, " +
      "move Kennel.app into /Applications, reopen it from there, and " +
  ...
  try {
    ...
    return (
      "The update can't be installed because Kennel's location isn't " +
      `writable: ${path.dirname(bundle)}. Fix that folder's permissions, or move ` +
      "Kennel.app somewhere you can write to, reopen it, and then " +
      "restart to update."
    );
  }
  return undefined;
...
export async function ensureUpdatePrefs(stateDir: string): Promise<void> {
  ...
  const optIn = await dialog.showMessageBox({
    ...
    defaultId: 0,
    cancelId: 1,
    message: "Keep Kennel up to date automatically?",
