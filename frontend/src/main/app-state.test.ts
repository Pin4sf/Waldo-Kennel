...
describe("writeAppStateMarker", () => {
	...
	beforeEach(async () => {
		dir = await mkdtemp(path.join(os.tmpdir(), "kennel-app-state-"));
	});
	...
	it("first write sets installedAt + installSource from installedVia", async () => {
		...
		await writeAppStateMarker({
			stateDir: dir,
			appPath: "/Applications/Kennel.app",
			version: "0.0.0",
		...
		expect(m.appPath).toBe("/Applications/Kennel.app");
	...
	it("second write PRESERVES installedAt/installSource and updates appPath/version/lastReconciledAt", async () => {
		await writeAppStateMarker({
			stateDir: dir,
			appPath: "/tmp/staging/Kennel.app",
			version: "0.0.0",
			installedVia: "npm-bootstrap",
...
describe("migration marker", () => {
	const dirs: string[] = [];
	async function tmp() {
		const dir = await mkdtemp(path.join(os.tmpdir(), "kennel-appstate-"));
		dirs.push(dir);
		return dir;
	}
