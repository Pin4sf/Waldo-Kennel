...
function signInFailureDetail(error: unknown): string {
  ...
  if (message.includes("network") || message.includes("fetch")) {
		return "Kennel could not reach WorkOS. Check your connection and try again.";
  }
	return "Kennel could not complete WorkOS sign-in. Please try again.";
}
  ...
  ipcMain.handle("cloud:signIn", async () => {
    if (!workos) {
      await dialog.showMessageBox({
        ...
        detail:
					"Set VITE_WORKOS_CLIENT_ID and restart Kennel to enable sign-in.",
      });
      return;
    }
