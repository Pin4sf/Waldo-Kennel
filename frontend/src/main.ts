...
ipcMain.handle("app:installTmux", async () => {
	if (process.platform !== "darwin") {
		return { status: "failed" as const, message: "Installing tmux via Homebrew is available only on macOS." };
	}
	const confirmation = await dialog.showMessageBox({
		type: "question",
		buttons: ["Install tmux", "Cancel"],
		defaultId: 0,
		cancelId: 1,
		message: "Install tmux via Homebrew?",
		detail: "Kennel will run:\n\nbrew install tmux\n\nHomebrew may download software and ask for your system password.",
	});
	if (confirmation.response !== 0) return { status: "cancelled" as const };

	const shellPath = cachedShellEnv?.PATH ?? process.env.PATH ?? "";
	const homebrewPath = ["/opt/homebrew/bin/brew", "/usr/local/bin/brew"].find((candidate) => existsSync(candidate));
	if (!homebrewPath) {
		return {
			status: "failed" as const,
			message: shellPath.includes("brew")
				? "Homebrew could not be started. Make sure brew is available in your login shell, then try again."
				: "Homebrew is not installed. Install Homebrew first, then try again.",
		};
	}

	return new Promise<{ status: "installed" | "failed"; message?: string }>((resolve) => {
		let output = "";
		let child: ChildProcess;
		try {
			child = spawn(homebrewPath, ["install", "tmux"], {
				env: cachedShellEnv ?? process.env,
				stdio: ["ignore", "pipe", "pipe"],
				windowsHide: true,
			});
		} catch (error) {
			resolve({ status: "failed", message: error instanceof Error ? error.message : "Could not start Homebrew." });
			return;
		}
		...
		child.once("close", (code) => {
			...
			resolve({
				status: "failed",
				message: detail ? `Homebrew could not install tmux: ${detail}` : "Homebrew could not install tmux.",
			});
		});
	});
});
