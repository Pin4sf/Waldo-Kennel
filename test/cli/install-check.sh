...
KENNEL_BIN="${KENNEL_BIN:-ao}"
tmp="$(mktemp -d)"
export KENNEL_RUN_FILE="$tmp/running.json"
export KENNEL_DATA_DIR="$tmp/data"
trap 'rm -rf "$tmp"' EXIT

fail() { echo "FAIL: $1" >&2; exit 1; }

echo "kennel binary : $(command -v "$KENNEL_BIN")"
"$KENNEL_BIN" version            >/dev/null || fail "version"
"$KENNEL_BIN" doctor             >/dev/null || fail "doctor"

# `kennel start` is now the desktop-app launcher: it resolves an installed app or
...
# with a clear `kennel start:` error (an unreachable/404 download on amd64, or an
...
if err="$("$KENNEL_BIN" start 2>&1)"; then
