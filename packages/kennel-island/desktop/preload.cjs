...
/**
 * Playhead fields, rebuilt one at a time like everything else that crosses the
 * bridge. All or nothing: a scrubber needs a position, a length and the moment
 * the position was true, and two out of three draws a bar that lies.
 */
function projectedProgress(track) {
	const position = Number(track?.positionSeconds);
	const duration = Number(track?.durationSeconds);
	const sampledAt = Number(track?.sampledAt);
...
function projectedMediaActivity(value) {
	if (!value || typeof value !== "object") return { playing: false, owner: null, track: null };

	const playing = value.playing === true;
	// The owner survives even when the title does not: it is what the island
	// names and focuses for a source that will not say what it is playing.
	const owner = projectedText(value.owner) || null;

	const title = projectedText(value.track?.title);
	if (!title) return Object.freeze({ playing, owner, track: null });

	const artwork = projectedArtwork(value.track?.artwork);
	const progress = projectedProgress(value.track);
	return Object.freeze({
		playing,
		owner,
		track: Object.freeze({
...
const SETTINGS_SHAPE = Object.freeze({
	notch: Object.freeze({ widthOffset: 0, heightOffset: 0, contentPadding: 8 }),
	hover: Object.freeze({
		peek: true,
		peekWidth: 14,
		peekHeight: 6,
...
const kennelDesktop = Object.freeze({
	...
	// Focus is requested only by a surface that needs a caret. Everything else
	// on the island is a click that must leave the key window where it was.
	setFocusable(focusable) {
		return ipcRenderer.invoke(IPC_SET_FOCUSABLE, { focusable: focusable === true });
	},
	...
	// No argument: the host already knows who is playing, and letting the
	// renderer name an application would make this a way to launch anything.
	// Seconds from the start of the track, checked here as well as in the main
	// process so the bridge only forwards a value the island could have made.
	seekMedia(positionSeconds) {
		const target = Number(positionSeconds);
		if (!Number.isFinite(target) || target < 0) return Promise.resolve({ sought: false });
		return ipcRenderer.invoke(IPC_SEEK_MEDIA, { positionSeconds: target });
	},
	focusMediaApp() {
		return ipcRenderer.invoke(IPC_FOCUS_MEDIA_APP);
	},
	recenter() {
