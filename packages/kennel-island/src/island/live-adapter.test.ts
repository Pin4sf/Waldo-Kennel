...
function createDesktopMock(
  ...
  const desktop: KennelDesktopAPI = {
    ...
    onStageGeometry: () => () => {},
    getMediaActivity: async () => ({ playing: false, owner: null, track: null }),
    onMediaActivity: () => () => {},
    focusMediaApp: () => Promise.resolve({ focused: false }),
    setFocusable: () => Promise.resolve({ focusable: false }),
    seekMedia: () => Promise.resolve({ sought: false }),
