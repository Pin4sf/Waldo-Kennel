...
export function describePushToggle(status: PushStatus | null, server: ServerTarget): PushToggle {
	...
	if (!hasServer(server)) {
		return {
			value: false,
			disabled: true,
			footer: "Connect to your Kennel server first — notifications turn on once connected.",
			blocked: false,
		};
	}
	if (!status.granted && !status.canAskAgain) {
		return {
			value: false,
			disabled: false,
			footer: "Notifications are turned off for Kennel in system settings.",
			blocked: true,
		};
	}
...
export type PushRegisterFailure =
	...
	| "not-configured" // no Kennel server paired yet, so there's nothing to register with
 ...
 * it: telling someone with a wrong password to "check that Kennel is running" sends
...
export function describeRegisterFailure(
	...
	switch (reason) {
		case "server-unreachable":
			return {
				title: "Couldn't reach your Kennel server",
				message:
					"Your device is set up for notifications, but we couldn't reach your server to register it. " +
					"Check that Kennel is running and your phone is on the same network, then try again.",
			};
		case "server-auth":
			return {
				title: "Your Kennel server rejected the request",
				message:
					"We reached your server, but it wouldn't accept the connection password. " +
					"Re-enter it under Settings → Connect Kennel, then try again.",
			};
		case "server-rate-limited":
			return {
				title: "Too many attempts",
				message: "Your Kennel server is temporarily refusing new attempts. Wait a minute, then try again.",
			};
		case "server-error":
			return {
				title: "Your Kennel server couldn't register this device",
				message:
					`We reached your server, but it returned an error${status ? ` (HTTP ${status})` : ""}. ` +
					"Check the Kennel logs on your computer, then try again.",
			};
		case "not-configured":
			return {
				title: "Connect to your Kennel server first",
				message:
					"This app isn't paired with a server yet, so there's nothing to register with. " +
					"Pair with your server under Settings → Connect Kennel — notifications turn on once connected.",
			};
		case "token-failed":
			return {
		...
		case "denied":
			return {
				...
				message: "Allow notifications for Kennel in your system settings, then try again.",
			};
		case "no-project-id":
			return {
